...
Burning cells remain active for duration $\tau_{\text{burn}}(\text{LandUse})$ before transitioning to state $2$ (Burnt):
...
stateDiagram-v2
    [*] --> UNAWARE
    UNAWARE --> PREPARING: Evacuation order received OR fire seen nearby
    UNAWARE --> ATRISK: Fire overtakes house before warning
    PREPARING --> WAITING: Packing delay elapsed (ready to drive)
    PREPARING --> ATRISK: Fire arrives during packing
    WAITING --> ONROAD: Road queue slot available
    WAITING --> ATRISK: Fire burns intersection
    ONROAD --> SAFE: Reached highway exit boundary
    ONROAD --> TRAPPED: Road link burned by fire
```

* **`UNAWARE` (0)**: Normal baseline activity.
* **`PREPARING` (1)**: Packaging essentials, loading vehicles, securing pets.
* **`WAITING` (2)**: Parked at driveway/curb, waiting for entry headway into local street.
* **`ONROAD` (3)**: In-transit along network links.
* **`SAFE` (4)**: Vehicle has crossed study area boundary exits.
...
The time required to mobilize and pack after receiving an alert is modeled as a log-normal distribution calibrated to empirical evacuation behavioral surveys:
$$\Delta t_{\text{pack}} \sim \text{Lognormal}(\mu, \sigma^2)$$

* **Daytime (Afternoon)**: Median $\approx 14\text{ min}$ ($\mu = 2.6, \sigma = 0.4$).
* **Nighttime (2:00 a.m.)**: Median $\approx 20\text{ min}$ ($\mu = 3.0, \sigma = 0.5$), representing sleep inertia and finding pets/keys in the dark.
...
* **Direct Official Broadcast (WEA / Reverse 911)**: $P_{\text{reach}} = 88\%$ during day, $65\%$ at 2 a.m. (phones set to Do Not Disturb).
* **Adaptive / Cue-Based Perception**: In adaptive mode, unaware households become aware via shared environmental fields:
  ...
  where $\text{Info}(c)$ represents social contagion (seeing neighbors pack vehicles and leave), generating realistic **shadow evacuation**.

---

## 4. Traffic Flow & Queue Dynamics

Traffic is simulated via a link-based spatial queue model (inspired by MATSim) that captures bottlenecks, queue spillback, and variable free-flow velocities without the computational overhead of micro-car-following differential equations.
...
* **Flow Capacity ($C_e$)**: Maximum household vehicles that can discharge into an downstream node per 10-second tick:
...
* **Storage Capacity ($S_e$)**: Maximum physical vehicle storage before spillback prevents upstream entry:
  $$S_e = \max\left(2.0, \frac{n_e \cdot L_e}{12.75}\right)$$
  assuming an effective vehicle footprint of $12.75\text{ m}$ (vehicle length + smoke safety spacing).
...
| :--- | :--- | :--- | :--- |
| $5z + 0$ | $d_{\text{fire}}(z)$ | Minimum Euclidean distance from fire perimeter to zone boundary | $d / 10\text{ km} \in [0, 1]$ |
| $5z + 1$ | $\text{Ordered}(z)$ | Evacuation order status ($1$ if ordered, $0$ if un-ordered) | $\{0, 1\}$ |
| $5z + 2$ | $\text{PctSafe}(z)$ | Fraction of zone population that has cleared exit boundaries | $[0, 1]$ |
| $5z + 3$ | $\text{PctFailed}(z)$ | Fraction of zone population trapped at home or on roads | $[0, 1]$ |
| $5z + 4$ | $\text{PctOnRoad}(z)$ | Fraction of zone population actively driving | $[0, 1]$ |
| $5N_z + 0$ | $ws$ | Wind speed | $ws / 80\text{ km/h}$ |
| $5N_z + 1$ | $\theta_w$ | Wind compass direction | $(\theta_w + \pi) / 2\pi$ |
| $5N_z + 2$ | $t_{\text{elapsed}}$ | Time since fire ignition | $t / 720\text{ ticks}$ |
| $5N_z + 3$ | $N_{\text{burning}}$ | Active fire front intensity (burning cell count) | $N_{\text{burn}} / 200$ |
| $5N_z + 4$ | $Q_{\max}$ | Maximum queue bottleneck ratio across key corridors | $\max(Q_e / C_e) / 5.0$ |
...
$$R_t = \underbrace{10.0 \cdot \Delta N_{\text{safe}}}_{\text{Reward for safe egress}} - \underbrace{500.0 \cdot \Delta N_{\text{casualties}}}_{\text{Severe penalty for entrapment/loss}} - \underbrace{0.01 \cdot \sum_{e \in E} \text{Queue}_e}_{\text{Penalty for road gridlock}} - \underbrace{\sum_{z} 2.0 \cdot \mathbb{I}(a_z = 1 \land d_{\text{fire}}(z) > 4.0\text{ km})}_{\text{Penalty for premature false alarms}}$$
...
$$L^{\text{TOTAL}}(\theta) = - L^{\text{CLIP}}(\theta) + c_1 L^{VF}(\theta) - c_2 S[\pi_\theta](s_t)$$
where $L^{VF}(\theta) = \frac{1}{2} (V_\theta(s_t) - V_t^{\text{targ}})^2$, entropy bonus $c_2 = 0.01$, and value coefficient $c_1 = 0.5$.
...
2. **In-Browser Execution Engine** ([`src/rl_policy.js`](file:///Users/rskris/dev_projects/agentsim_wildfire/src/rl_policy.js)):
   ...
   This runs in **$< 0.1\text{ ms}$** on the client thread, enabling instantaneous real-time decisions without WebAssembly overhead or server network calls.
